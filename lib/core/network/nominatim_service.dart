...
/// Broad classification of a Nominatim result, derived from its geometry
/// (falling back to its OSM class when no geometry was returned) — used to
/// decide how the result should be drawn and whether it filters incidents.
...
/// Geocoding client for Nominatim/OpenStreetMap. Kept separate from
/// [ApiClient] since it targets a different host and needs no cookies.
class NominatimService {
  ...
  static final Dio _dio = Dio(
    BaseOptions(
      ...
      headers: {'User-Agent': 'ArconteApp/1.0 (citizen safety reporting app)'},
    ),
  );

  // Rough bounding box around Cochabamba, Bolivia used to bias (not
  // restrict — `bounded=0`) results toward the app's home region.
